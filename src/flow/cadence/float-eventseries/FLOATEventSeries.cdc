...
access(all) contract FLOATEventSeries {
    ...
    access(all) struct TokenDefinition {
        ...
        view init(type: Type, path: PublicPath, isNFT: Bool) {
            self.type = type
            self.path = path
            self.isNFT = isNFT
        }
    ...
    access(all) view fun getTokenDefinition(_ token: Type): TokenDefinition? {
    ...
    access(all) struct TokenRecipient {
        ...
        view init(_ address: Address, _ identifier: Type) {
        ...
        access(contract) fun getFungibleTokenReceiver(): &{FungibleToken.Receiver} {
            ...
            let receiverVault = getAccount(self.address)
                .capabilities
                .borrow<&{FungibleToken.Receiver}>(tokenInfo.path)
        ...
        access(contract) view fun getNFTCollectionPublic(): &{NonFungibleToken.CollectionPublic} {
            let tokenInfo = FLOATEventSeries.getTokenDefinition(self.identifier) ?? panic("Unknown token")
            assert(tokenInfo.isNFT, message: "The token should be Non-Fungiable Token")

            let collection = getAccount(self.address)
                .capabilities
                .borrow<&{NonFungibleToken.CollectionPublic}>(tokenInfo.path)
    ...
    access(all) struct EventIdentifier {
        ...
        view init(_ address: Address, _ eventId: UInt64) {
            self.host = address
            self.eventId = eventId
        }

        // get the reference of the given event
        access(all) fun getEventPublic(): &FLOAT.FLOATEvent {
            let ownerEvents = getAccount(self.host)
                .capabilities
                .borrow<&FLOAT.FLOATEvents>(FLOAT.FLOATEventsPublicPath)
    ...
    access(all) struct EventSeriesIdentifier {
        ...
        access(all) view fun getEventSeriesPublic(): &FLOATEventSeries.EventSeries {
            let ref = getAccount(self.host)
                .capabilities
                .borrow<&EventSeriesBuilder>(FLOATEventSeries.FLOATEventSeriesBuilderPublicPath)
    ...
    access(all) struct interface EventSlot {
        ...
        access(all) view fun getIdentifier(): EventIdentifier?
        // if the event is required for achievement
        access(all) view fun isEventRequired(): Bool
    ...
    access(all) struct interface IAchievementGoal {
        ...
        access(all) view fun getPoints(): UInt64 {
            post {
    ...
    access(all) struct StrategyDeliveryFTWithRandomAmount: StrategyDelivery {
        ...
        access(contract) fun deliverFT(treasury: &Treasury, recipient: &{FungibleToken.Receiver}): UFix64 {
            ...
            if self.maxClaimableShares == self.claimedShares + 1 {
                ...
                let rnd = revertibleRandom<UInt64>(modulo: 100)
                randShareAmount = oneShareAmount + oneShareAmount * UFix64(rnd) / 100.0
            }
    ...
    access(all) struct StrategyDeliverNFT: StrategyDelivery {
        ...
        access(contract) fun deliverNFT(treasury: &Treasury, recipient: &{NonFungibleToken.CollectionPublic}): [UInt64] {
            ...
            let rnd = revertibleRandom<UInt64>(modulo: UInt64(ids.length))
            let nft <- treasuryRef.withdraw(withdrawID: ids.remove(at: rnd))
    ...
    access(all) resource StrategyController {
        ...
        access(all) view fun getInfo(): StrategyInformation {
            return self.info
        }

        // get current state of the strategy
        access(all) view fun getCurrentState(): StrategyState {
            return self.info.currentState
        }

        // get total shares of the strategy
        access(all) view fun getTotalShares(): UInt64 {
            return self.info.delivery.maxClaimableShares
        }

        // get claimed shares of the strategy
        access(all) view fun getClaimedShares(): UInt64 {
            return self.info.delivery.claimedShares
        }

        // get claimed addresses
        access(all) view fun getClaimedAddresses(): [Address] {
            return self.claimed
        }

        // if user has claimed
        access(all) view fun hasClaimed(address: Address): Bool {
        ...
        // claim one share
        access(contract) fun claimOneShareFromTreasury(treasury: &Treasury, user: &Achievement) {
            pre {
        ...
        access(contract) view fun verifyScore(user: &Achievement): Bool {
    ...
    access(all) resource interface ITreasuryStrategy {
        ...
        access(account) fun isEligible (user: &Achievement): Bool

        // update user's achievement
        access(account) fun onGoalAccomplished(user: &Achievement) {
            pre {
        ...
        access(account) fun verifyClaimable(user: &Achievement): Bool {
            pre {
    ...
    access(all) resource TreasuryCollection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        access(all) view fun getLength(): Int {
            return self.depositedNFTs.length
        }
        ...
        access(all) fun forEachID(_ f: fun (UInt64): Bool): Void {
            self.depositedNFTs.forEachKey(f)
        }
    ...
    access(all) resource interface TreasuryPublic {
        ...
        access(all) fun refreshUserStatus(user: &Achievement)
        ...
        access(all) fun claim(strategyIndex: Int, user: &Achievement)
    ...
    access(all) resource Treasury: TreasuryPublic, Burner.Burnable {
        // Treasury seriesID
        ...
        access(all) fun depositNonFungibleTokens(nfts: @[{NonFungibleToken.NFT}]) {
            ...
            let ids: [UInt64] = []
            var collectionRef: &{NonFungibleToken.CollectionPublic}? = &self.genericNFTPool[nftType]
    ...
    access(all) resource interface EventSeriesPublic {
        ...
        access(all) view fun getIdentifier(): EventSeriesIdentifier
        // get last slot index
        access(all) view fun getLastSlotIdx(): Int
        // get all slots data
        access(all) view fun getSlots(): [{EventSlot}]
        // get a event slot by index
        access(all) view fun getSlot(idx: Int): {EventSlot}
        // get all goals data
        access(all) view fun getGoals(): [{IAchievementGoal}]
        // get an achievement goal by index
        access(all) view fun getGoal(idx: Int): {IAchievementGoal}
        // get extra information of event series
        access(all) view fun getExtra(): {String: AnyStruct}
        // borrow the treasury public reference
        access(all) view fun borrowTreasuryPublic(): &Treasury
    ...
    access(all) resource EventSeries: EventSeriesPublic, ViewResolver.Resolver {
        access(all) let sequence: UInt64
        access(all) let host: Address
        // --- basics ---
        access(all) var name: String
    ...
    access(all) resource interface EventSeriesBuilderPublic {
        ...
        access(all) view fun getEventSeriesIDs(): [UInt64]
        // check if some id is revoked
        access(all) view fun isRevoked(seriesId: UInt64): Bool
        // borrow the public interface of EventSeries
        access(all) view fun borrowEventSeriesPublic(seriesId: UInt64): &EventSeries?
        // internal full reference borrowing
        access(account) view fun borrowEventSeriesBuilderFullRef(): &EventSeriesBuilder
    }

    // the event series resource collection
    access(all) resource EventSeriesBuilder: EventSeriesBuilderPublic, ViewResolver.ResolverCollection {
        access(all) let sequence: UInt64
        ...
        access(all) fun registerToken(path: PublicPath, isNFT: Bool) {
            ...
            if isNFT {
                ...
                let ft = self.owner!
                    .capabilities
                    .borrow<&{FungibleToken.Receiver}>(path)
        ...
        access(all) view fun borrowEventSeries(seriesId: UInt64): &EventSeries? {
            return &self.series[seriesId]
        }
    ...
    access(all) resource interface AchievementBoardPublic {
        ...
        access(all) view fun borrowAchievementRecordRef(host: Address, seriesId: UInt64): &Achievement?
    }
    ...
    access(all) fun borrowEventSeriesGlobal(): &EventSeriesGlobal {
        return self.account
            .storage
            .borrow<&EventSeriesGlobal>(from: self.FLOATEventSeriesGlobalStoragePath)
    ...
    init() {
        ...
        self.account.storage.save(<- create EventSeriesGlobal(), to: self.FLOATEventSeriesGlobalStoragePath)
        self.account.capabilities.publish(
            self.account.capabilities.storage.issue<&EventSeriesGlobal>(self.FLOATEventSeriesGlobalStoragePath),
            at: self.FLOATEventSeriesGlobalPublicPath
        )
